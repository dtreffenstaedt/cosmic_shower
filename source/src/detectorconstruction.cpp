...
G4VPhysicalVolume* DetectorConstruction::Construct()
{
    ...
    G4Box* world_box = new G4Box("World", world_length, world_length, world_length);
    ...
    G4Solid* box_layer[12]; // geometry of each layer
    ...
    for (size_t i = 0; i < 12; i++)
    {
        ...
        material_layers[i] = new G4Material("material_layer" + std::to_string(i), rho_0 * layers_factor[i], air);
        box_layer[i] = new G4Box("box_layer" + std::to_string(i), world_side * 0.5, world_side * 0.5, layer_thickness * 0.5);
        logical_layer[i] = new G4LogicalVolume(box_layer[i], material_layers[i], std::to_string(i));
        physical_layer[i] = new G4VPlacement(
                0,
                G4ThreeVector(
                    0.0 * m,
                    0.0 * m,
