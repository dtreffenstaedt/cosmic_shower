...
void SteppingAction::UserSteppingAction(const G4Step* step)
{
    ...
    if (step->GetPreStepPoint()->GetTouchableHandle()->GetVolume()->GetLogicalVolume()->GetName() == "Intensitycatcher") {
        ...
        RecorderManager::singleton()->store_momentum(step->GetPreStepPoint()->GetPosition(), track->GetParticleDefinition()->GetPDGMass(), track->GetKineticEnergy(), (track->GetParticleDefinition()->GetPDGCharge()) > std::numeric_limits<double>::epsilon() );
